				...
				while (!net.stop) {
					...
					try {
						...
						if (net.inputdata.petriObject.GetType() == PetriObjectType.PetriData) {
							...
							if(net.inputdata.petriObject instanceof DataCar && net.PlaceList.get(index).GetType() == PetriObjectType.DataCarQueue) {
								((CarQueue) net.PlaceList.get(index).GetValue()).AddCar((DataCar) net.inputdata.petriObject);
							} else {
