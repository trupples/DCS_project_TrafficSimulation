...
public class Condition implements Serializable {
	...
	boolean Check() {
		...
		case NotEqual:
			...
			break;
		case Equal:
			System.out.println("Comparing: " + Value1 + " ; " + Value2);
			if (Value1 == null || Value2 == null)
				return false;
			if (Value1.GetValue() == null || Value2.GetValue() == null)
				return false;
		...
		case HaveCar: {
			...
			if (Value1.GetValue() == null)
				return false;
			//System.out.println("Value1.GetType() = " + Value1.GetType());
			if (Value1.GetType() == PetriObjectType.DataCarQueue) {
				//System.out.println(((CarQueue) Value1.GetValue()).Cars);
				if (!((CarQueue) Value1.GetValue()).Cars.isEmpty()) {
					return true;
				}
			}
			break;
		...
		// ---------------------End of Modification-------------------------------
			...
			case NextCarGoesTo:
				if(Value1 == null || Value1.GetValue() == null || Value1.GetType() != PetriObjectType.DataCarQueue)
					return false;

				//System.out.println("Checking if car from " + Value1.GetName() + " to " + PlaceName2);
				//System.out.println("Car: " + ((CarQueue) Value1.GetValue()).Cars.peek());

				return ((CarQueue) Value1.GetValue()).FirstCarGoesTo(PlaceName2);

			case HasNoCar:
				if(Value1 == null || Value1.GetValue() == null || Value1.GetType() != PetriObjectType.DataCarQueue)
					return false;

				return ((CarQueue) Value1.GetValue()).Cars.isEmpty();
	...
	public boolean CheckCondition() {
		...
		for (Condition condition : conditionList) {
			if (condition.Connector == LogicConnector.AND) {
				if (!condition.Check()) {
					System.out.println("(" + Parent.TransitionName + ") AND condition failed: " + condition.condition);
					return false;
					//andCondition = false;
					//break;
				}
		...
		if (andCondition) {
			System.out.println(Parent.TransitionName + " succeeded on ANDs :)");
			return true;
		}
		...
		for (Condition condition : conditionList) {
			if (condition.Connector == LogicConnector.OR) {
				if (condition.Check()) {
					System.out.println("(" + Parent.TransitionName + ") OR condition succeeded: " + condition.condition);
					return true;
				}
			}
		}
