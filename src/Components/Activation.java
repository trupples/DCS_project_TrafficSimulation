...
public class Activation implements Serializable {
	...
	public void Activate() throws CloneNotSupportedException {
		...
		if (Operation == TransitionOperation.PopElement)
			PopElement();

		if (Operation == TransitionOperation.PopElementToQueue)
			PopElementToQueue();
...
//			PetriObject temp = Parent.TempMarking.get(inputIndex);
	...
	private void PopElement() throws CloneNotSupportedException {
	...
	private void PopElementToQueue() throws CloneNotSupportedException {
		Integer outputIndex = util.GetIndexByName(OutputPlaceName, Parent.Parent.PlaceList);
		Integer inputIndex = util.GetIndexByName(InputPlaceName, Parent.Parent.PlaceList);
		PetriObject temp = ((CarQueue) ((DataCarQueue) Parent.Parent.PlaceList.get(inputIndex)).GetValue())
				.PopCar();
	...
	private void SendOverNetwork() throws CloneNotSupportedException {
		...
		if (inputIndex == -1) {
			...
			temp = Parent.Parent.PlaceList.get(inputIndex);
		}
...
//		PetriObject temp = Parent.TempMarking.get(inputIndex);
		...
		if (temp instanceof DataCarQueue) {
			DataInteger count = new DataInteger();
			int c = ((CarQueue) temp.GetValue()).Cars.size();
			count.SetValue(c);
			result.SetValue((PetriObject) count);
		}
